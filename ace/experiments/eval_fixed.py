...
def main():
    ...
    runs_dir = f"runs/eval_fixed/{opponent_name}"
    logger.set_level(logger.DEBUG)

    agent = Planner(player_id=0, map_name=map_name, model=cfg.agents[0].model, prompt="few-shot-w-strategy", temperature=cfg.agents[0].temperature, max_tokens=cfg.agents[0].max_tokens, strategy="ace/data/strategies/strategy_23.json")
    ...
    for episode in range(cfg.episodes):
        ...
        print(f"FIXED | Match {episode} | Opponent {opponent_name} | Payoffs: {payoffs} | Runtime: {(time.time() - start_time) / 60:.2f}min, {env.time}steps")
