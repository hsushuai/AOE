...
def main():
    ...
    cfg = parse_args()
    map_name = cfg.env.map_path.split("/")[-1].split(".")[0]
    runs_dir = f"pre_match_runs/{cfg.agents[1].strategy.split('/')[-1].split('.')[0]}"
    ...
    while payoffs[0] <= 0 and cfg.max_iterations > i:
        ...
        trajectory = trajectory.to_string() if hasattr(trajectory, "to_string") else ""
        ...
        print(f"Iteration {i} | Payoffs: {payoffs}")
        i += 1
