...
if __name__ == "__main__":
    ...
    envs = MicroRTSGridModeVecEnv(
        ...
        max_steps=2000,
        render_theme=2,
        ai2s=[bot_agent.coacAI for _ in range(args.num_bot_envs - 6)]
        + [bot_agent.randomBiasedAI for _ in range(min(args.num_bot_envs, 2))]
        + [bot_agent.lightRushAI for _ in range(min(args.num_bot_envs, 2))]
        + [bot_agent.workerRushAI for _ in range(min(args.num_bot_envs, 2))],
    ...
    trueskill_writer = TrueskillWriter(
        args.prod_mode, writer, "~/desc/skill-rts/gym_microrts/gym-microrts-static-files/league.csv", "~/desc/skill-rts/gym_microrts/gym-microrts-static-files/league.csv"
    )
