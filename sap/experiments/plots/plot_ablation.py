...
import numpy as np
from sap.experiments.plots.plot_main_result import calculate_payoffs
...
def get_results():
    ...
    stat = {}
    seen = [filename.split(".")[0] for filename in os.listdir("sap/data/train")]
    unseen = [filename.split(".")[0] for filename in os.listdir("sap/data/test")]
    ...
    df.to_csv("sap/experiments/plots/ablation.csv")
...
def plot():
    ...
    ax.bar(x - 1.5 * width, [seen_percentage[0], unseen_percentage[0]], width, label="SAP", color=colors[0], edgecolor="black", hatch="/")
    ax.bar(x - 0.5 * width, [seen_percentage[1], unseen_percentage[1]], width, label="SAP w/o greedy", color=colors[1], edgecolor="black", hatch="x")
    ax.bar(x + 0.5 * width, [seen_percentage[2], unseen_percentage[2]], width, label="SAP w/o SEN", color=colors[2], edgecolor="black", hatch="\\")
    ax.bar(x + 1.5 * width, [seen_percentage[3], unseen_percentage[3]], width, label="SAP fixed", color=colors[3], edgecolor="black", hatch="-")
    ...
    # Save plot
    plt.tight_layout()
    plt.savefig("sap/experiments/plots/ablation.pdf")
