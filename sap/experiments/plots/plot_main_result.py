...
def get_main_result(win_loss_only):
    ...
    for opponent, payoffs in ace_results.items():
        df.loc["SAP", opponent] = payoffs[0]
        df.loc[opponent, "SAP"] = payoffs[1]
    
    df = df.fillna(0)
    order = ["Vanilla", "CoT", "PLAP", "Seen", "Unseen", "SAP"]
    df = df.reindex(index=order, columns=order, fill_value=0)
    df.to_csv("sap/experiments/plots/main_results.csv")


def get_scaling_result():
    results = get_ace_result("runs/scaling_runs_error_net", win_loss_only=True)
    df = pd.DataFrame()
    ...
    order = ["Vanilla", "CoT", "PLAP", "Seen", "Unseen", "SAP"]
    df = df.reindex(index=order, columns=order, fill_value=0)
    df.to_csv("sap/experiments/plots/scaling_results.csv")
