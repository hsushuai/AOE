...
def eval_model():
    ...
    model = PayoffNet.load("sap/data/payoff/payoff_net.pth")

    feat_space = Strategy.feat_space()
    
    opponents = os.listdir("sap/data/test")
    for opponent in opponents:
        run_dir = f"{runs_dir}/{opponent}"
        
        # Search for the best response to the opponent
        opponent_strategy = Strategy.load_from_json(f"sap/data/test/{opponent}")
